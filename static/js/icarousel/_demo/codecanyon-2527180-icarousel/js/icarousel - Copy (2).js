...
(function($) {
	...
	var iCarouselStarter = function(el, slides, options){
		...
		ImagePreload.prototype.OnAbort=function(){this.bAbort=true;this.oImagePreload.OnComplete()}
			...
			iCarouselPreloader.css({ top:((defs.height/2)-(iCarouselPreloader.height()/2))+'px', left:((defs.width/2)-(iCarouselPreloader.width()/2))+'px' });
			...
			if(defs.timer=="bar") {
				...
				barTimer.css({ width:0, height:stroke, background:options.timerColor, 'float':'left' });
			} else {
			...
			var iCarousel = {
				...
				processTimer: function(){
					if(defs.timer=="360bar") {
						...
						timerPath.attr({arc: [degree, R]});
					} else if(defs.timer=="pie") {
						var degree = (defs.degree==0) ? 0 : defs.degree-.9;
						pieTimer.attr({segment: [degree, R]});
					} else {
						barTimer.css({ width: ((this.defs.degree/360)*100)+'%' });
					}
					defs.degree += 4;
				},
				...
				resetTimer: function(){
					if(defs.timer=="360bar") {
						timerPath.animate({ arc: [0, R] }, options.animationSpeed);
					} else if(defs.timer=="pie") {
						pieTimer.animate({ segment: [0, R] }, options.animationSpeed);
					} else {
						barTimer.animate({ width: 0 }, options.animationSpeed);
					}
				},
				...
				layout: function(animate, speedTime){
					...
					slides.css({ top: slideTop+'px', position: 'absolute', opacity: 0, visibility: 'hidden' });
				...
				setButtons: function(){
					el.append('<a class="iCarouselNav" id="iCarouselPrev">'+options.previousLabel+'</a><a class="iCarouselNav" id="iCarouselNext">'+options.nextLabel+'</a>');
				}
			};
