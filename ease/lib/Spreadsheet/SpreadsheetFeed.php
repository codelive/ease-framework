...
class SpreadsheetFeed extends \ArrayIterator
{
	...
	public function getIdByTitle($title) {
		foreach($this->xml->entry as $entry) {
			if($entry->title->__toString()==$title) {
				$spreadsheet_url = trim($entry->link[1]->attributes()->href);
				if(preg_match('@^https://docs\.google\.com/(.*?)spreadsheets/d/(.*)/edit$@i', $spreadsheet_url, $matches)) {
					$id = $matches[2];
				} else {
					$spreadsheet_url_parts = explode('?key=', $spreadsheet_url);
					$id = $spreadsheet_url_parts[1];
				}
				if(trim($id)!='') {
					return $id;
				}
